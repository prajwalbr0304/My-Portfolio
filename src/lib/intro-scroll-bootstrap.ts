...
export const PORTFOLIO_RELOAD_TO_ROOT_BOOTSTRAP = `(function(){try{if(typeof sessionStorage==="undefined"||typeof window==="undefined"||typeof performance==="undefined")return;if(window.matchMedia&&window.matchMedia("(max-width: 767px), (pointer: coarse)").matches)return;var nav=performance.getEntriesByType&&performance.getEntriesByType("navigation")[0];var isReload=(nav&&nav.type==="reload")||(typeof performance.navigation!=="undefined"&&performance.navigation.type===1);if(!isReload)return;var k=${JSON.stringify(INTRO_DISMISSED_SESSION_KEY)};try{sessionStorage.removeItem(k);}catch(e){}var p=window.location.pathname||"";if(p==="/portfolio"||p.indexOf("/portfolio/")===0){window.location.replace("/");}}catch(e){}})();`;

/** @deprecated Use ROOT_BEFORE_INTERACTIVE_BOOTSTRAP */
export const PORTFOLIO_INTRO_SCROLL_LOCK_BOOTSTRAP = `(function(){try{if(typeof document==="undefined"||typeof window==="undefined")return;var path=window.location.pathname||"";if(path!=="/")return;if(window.matchMedia("(prefers-reduced-motion: reduce)").matches)return;if(window.matchMedia("(max-width: 767px), (pointer: coarse)").matches)return;var k=${JSON.stringify(INTRO_DISMISSED_SESSION_KEY)};var nav=performance.getEntriesByType&&performance.getEntriesByType("navigation")[0];var isReload=(nav&&nav.type==="reload")||(typeof performance.navigation!=="undefined"&&performance.navigation.type===1);if(isReload){try{sessionStorage.removeItem(k);}catch(e){}}try{if(sessionStorage.getItem(k)==="1"){document.documentElement.removeAttribute("data-portfolio-intro-lock");return;}}catch(e){}document.documentElement.setAttribute("data-portfolio-intro-lock","");}catch(e){}})();`;
